...
In all previous examples, we looked at single sample data, which are either vector or single-column matrix. \Rpackage{Pathview} also handles multiple sample data, generates graph for each sample. It automatically match samples by recycling over the smaller sample size when sample sizes are different for gene and compound data. In the following example (Figure not shown), \R{gene.data} has two samples while \R{cpd.data} has one. Hence \R{cpd.data} is recycled to match the second \R{gene.data} sample in the second graph. To prevent such data matching/recycling, you need to set argument \R{match.data=F}, which is passed to secondary function \R{keggview.native} or \R{keggview.graph} by \R{pathview}.
...
#number of species which use other ID types as the default ID
...
Then, we work on E coli gene data with ID types other than the default KEGG ID, for example, official gene symbols. When calling \R{pathview}, such data need to be mapped to Entrez Gene ID first (if not yet), then to default KEGG ID (Locus tag). Therefore, it takes longer time than the last example.
