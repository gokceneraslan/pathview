...
#map between cpd ids, both kegg and others
  ...
  if(in.type %in% cpd.type[kg.idx]) {
    message("A native KEGG compound ID type, no need to map!")
    return(invisible(0))
  }
...
#  in.ids=unique(in.ids)
  ...
  if(sum(in.idx)<1) {#stop("None of the compound ids mapped to the specified type!")
    ...
    if(sum(rev.idx)>sum(in.idx)) message("multiple compounds may map to a input ID, only the first one kept!")
    ...
    if(sum(map.idx)<1) message("None of the compound ids mapped to specified output ID type(s)!")
